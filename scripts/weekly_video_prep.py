...
def search_company_direction():
    ...
    try:
        ...
        q = (
            ...
            f"검색 결과 기반 사실만, 각 25자 내외 한국어 구절 4~5개를 '/'로 구분해 한 줄로. "
            f"각 구절엔 '무엇을 위한 것인지'(목적·의미)가 드러나야 하고, 일반인이 모를 약어는 쉬운 말로 풀어 쓴다. "
            f"예: '메모리 반도체가 주력 사업 / AI용 메모리 1위 굳히기 전략 / 생산 확대에 20조 원 투자'"
        )
        client   = genai.Client(api_key=GEMINI_API_KEY)
        response = client.models.generate_content(
            model="gemini-2.5-flash",
...
• 각 줄은 **그 줄만 들어도 이해**되게 쓴다. 맥락 없이 결과·수치만 던지지 않는다 —
  수치가 나오면 '무엇을 위해/무슨 의미인지'가 같은 문장 안에 보여야 한다.
  나쁜 예: "한울반도체 지분 80억 취득했거든요" (80억이 갑자기 왜?)
  좋은 예: "80억을 들여 반도체 회사 지분을 사며 사업을 넓혔어요"
• 전문용어·영문 약어(QKD-PQC, POTN, HBM 등)는 그대로 나열하지 말고 **쉬운 말로 풀거나
  짧은 설명을 붙인다**: "QKD-PQC 결합" → "양자암호 통신 기술", "HBM" → "AI용 메모리(HBM)".
  한 줄에 일반인이 모를 약어는 최대 1개, 반드시 풀이와 함께.
• 시청자가 "그래서 그게 뭔데?"라고 되묻게 되는 줄은 실패다 — 쓰기 전에 스스로 이 질문으로 검증한다.
...
※ 뉴스 감(感)이 아니라 실제 수치(추세·밸류·목표주가·수급·재무제표)로 이 종목을 짚어주는 씬이다.
  ...
  "RSI 20.9" → "단기 과매도 신호", "PER 11.5배"처럼 수치 자체는 남기되 "이익 대비 싼 편"
  같은 짧은 풀이를 붙인다).
...
4. 알 수 없는 전문용어: 일반 시청자가 모를 약어·전문용어(QKD-PQC, POTN 등)가 풀이 없이 나오면
   쉬운 말로 바꾸거나 짧은 풀이를 붙인다("양자암호 통신", "AI용 메모리(HBM)" 식). 이 점검은 모든 씬에 적용한다.
   특히 기술적 지표 용어(RSI·MACD·PER·PBR·ROE·이동평균선 등 숫자+영문 약어)가 풀이 없이 그대로
   보이면 반드시 고친다 — 씬2·씬3에서 자주 새는 실수다.
   (예: "RSI 20.9 과매도" → "단기 과매도 신호", "PER 11.5배" → "PER *11.5배*(주가가 이익 대비
   싼 편)", "20일선 위" → 그대로 두되 처음 나올 때만 "20일 평균 주가선" 식으로 짧게 풀이)
...
def _fmt_krw_compact(v) -> str:
    ...
    try:
        ...
        return str(v)
    sign = "-" if v < 0 else ""
    eok = abs(v) / 100_000_000   # 억원 단위
    if eok >= 10000:
        return f"{sign}{eok / 10000:.1f}조원"
    return f"{sign}{eok:,.0f}억원"


def _format_signals(sig: dict) -> str:
    """enrich_signals.py의 정량 신호(technicals/supply_demand/valuation/analyst_reports/
    dart_financials)를 대본 프롬프트용 자연어 한 줄로 정리. 비면 '데이터 없음'.
    목표주가: 미국은 애널리스트 컨센서스 범위(평균·최고·최저), 한국은 네이버 금융
    증권사 리포트(제목에 명시된 경우만 숫자 포함 — 없는 리포트는 증권사·날짜만).
    재무제표(dart_financials): DART 주요계정 그대로, 지어낸 숫자 없음."""
    if not sig:
        return "데이터 없음"
    parts = []
    ...
    dart = sig.get("dart_financials") or {}
    if dart:
        period = dart.get("period", "")
        fin = [f"{k} {_fmt_krw_compact(v)}" for k, v in dart.items() if k != "period"]
        if fin:
            parts.append(f"[재무제표({period})] " + ", ".join(fin))
