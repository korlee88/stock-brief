...
- **주간 주요 일정**(weekly_calendar.py, 사용자 요청 — 카톡으로 받고 싶음): 일요일 22시 KST
  ...
  **2차 검증**(`OPENAI_API_KEY`, 선택·사용자 요청): impact="high" 항목만 OpenAI(web_search
  그라운딩, Structured Outputs)로 독립 재조사해 같은 날짜에 겹치는 일정을 못 찾으면
  medium으로 하향(`reconcile_high_impact`) — 서로 다른 두 검색 소스가 못 찾으면 근거
  불충분으로 본다. 키 없거나 실패해도 Gemini 결과만으로 정상 진행(필수 아님).
  **수집 실패 폴백**(사용자 확정 — 2026-09-21 주에 Gemini 1건/OpenAI 11건이 나와
  40자 메시지가 발송된 사건): Gemini 수집이 OpenAI의 절반 미만이면
  (`gemini_collection_broken`, OpenAI가 3건 이상일 때만) 수집 실패로 보고 OpenAI
  결과로 **대체**한다. 이때 교차 검증은 건너뛴다 — 깨진 목록과 대조하면 멀쩡한
  high가 전부 medium으로 깎인다. 대체 사실은 ops-log에 기록.
  **운영 로그**(사용자 요청 — "언제부터 이랬는지" 기억에 의존하지 않으려고): 일정 수집
  실패·OpenAI 2차 검증 실패·카카오 발송 실패 등 실제 이상 상황만 `log_incident()`가
  `data/ops-log.md`에 한 줄씩 누적(커밋됨). 키 미설정 같은 정상 스킵은 기록 안 함(노이즈 방지).
